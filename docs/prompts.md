...
### P-036 · Integração real com o Gemini e calibração dos limiares (issue #11, conclusão)
- **Origem:** usuário ("crie o .env, a api do gemini é … e o modelo é o gemini-2.5-flash" → 403 de cobrança → "fiz o pagamento, tente novamente") + reconstruído para a parte de calibração e evidências
- **Objetivo:** substituir o FakeLLM pelo modelo real no caminho de produção, provar o comportamento ponta a ponta e calibrar os limiares com dados reais (RNF-03, RNF-04, RNF-09; Q1 do PRD).
- **Prompt:**
  > Crie o .env a partir do .env.example com a GOOGLE_API_KEY e LLM_MODEL=gemini-2.5-flash, confirme que o git o ignora e valide a chave. Se o provedor recusar, diagnostique se o bloqueio é da chave, do modelo ou do projeto e diga o que fazer. Com a chave funcionando: rode `pytest -m live`, execute os 7 exemplos e o cenário SIMULAR_FALHA_TOOL=1 com o Gemini, salve o ResultadoTriagem e o log JSONL de cada um em docs/evidencias/, verifique que nenhum arquivo contém a chave ou dado pessoal real, e escreva um README com a tabela de resultados, o que funcionou e os problemas reais observados, separando o que é entrada para o refinamento de prompt (#16) e para a revisão de regras (#17). Adicione impacto, serviço e palavras-chave ao log de analisar_chamado para dar base à calibração. Calibre LIMIAR_BM25 medindo os scores da consulta real (título + descrição + palavras-chave do modelo) contra a base, inclusive para chamados curtos; só mude LIMIAR_CONFIANCA se os dados justificarem. Atualize config, .env.example, PRD (Q1, seção 14, RF-31) e docs/cenarios.md, e rode de novo os cenários afetados para provar o efeito da calibração.
- **Resultado:** chave inicialmente recusada com `403 PERMISSION_DENIED` (bloqueio de cobrança do projeto Google; resolvido pelo aluno). Depois: `pytest -m live` verde (2 testes), 8 execuções reais sem exceção e saída estruturada válida em 14 de 14 chamadas. `LIMIAR_BM25` calibrado de 1.0 para **12.0** (artigo correto 35–46, ruído até 11, fora do domínio 4,8, pior caso curto relevante 14,4); `LIMIAR_CONFIANCA` mantido em 0.6 porque o prompt v1 satura a confiança em 1.0 (recalibrar na #16). Problemas reais registrados como entrada das issues #16 e #17: exemplo 01 em rota crítica (regra RF-44(a) não considera impacto), confiança saturada, passos da ação sem separador, equipe inventada no exemplo 07. Log de retry após um 403 real preservado como evidência da RNF-03. 228 testes offline continuam verdes.

---

## Fase 5 — Documentação e evidências (iniciada em 15/09/2026)

### P-037 · QA com IA: revisão crítica das regras e da tool com decisão do aluno (issue #17)
- **Origem:** usuário ("pode seguir", após a proposta de tratar #17 e #16 nesta ordem) + prompt sugerido na issue, ajustado ao que foi feito
- **Objetivo:** critério 10 da rubrica: usar IA para revisar código real e gerar testes, registrando sugestão, decisão do aluno e motivo, com pelo menos uma sugestão rejeitada.
- **Prompt:**
  > Revise criticamente src/triagem/regras.py e src/triagem/tools/catalogo.py à luz das execuções reais de docs/evidencias/execucoes/prompt-v1/. Para cada hipótese que levantar lendo o código, confirme empiricamente antes de me apresentar (rode combinações, frases de teste, fragmentos). Aponte bugs, código morto, casos não cobertos e riscos de falso positivo, proponha testes adicionais para T9 e T5 e apresente as decisões que são minhas com opções e uma recomendação. Depois de eu decidir, aplique as aceitas, fixe por teste e comentário as rejeitadas, atualize o PRD, rode de novo o exemplo 01 com o Gemini para provar a correção e escreva docs/qa-com-ia.md com trecho, problema, sugestão, decisão, motivo e o commit onde a decisão foi aplicada.
- **Resultado:** 4 decisões do aluno (3 aceitas, 1 rejeitada) e 3 achados aditivos aceitos, documentados em `docs/qa-com-ia.md`. RF-44(a) passou a exigir impacto além de um usuário (o exemplo 01 voltou à rota simples com o modelo real, evidência em `pos-qa-regras/`); `definir_rota` perdeu dois ramos inalcançáveis e o motivo cita a regra que elevou; a detecção de termos por substring foi mantida como limitação documentada e fixada por teste; três apelidos genéricos saíram do catálogo; colisão de apelidos falha na carga; redação de segredos cobre fragmentos. 368 testes (140 novos). Commit `d004bc8`.

### P-038 · Instruções do agente documentadas e refinamento real de prompt v1 → v2 (issue #16)
- **Origem:** reconstruído (prompt sugerido na issue #16, ajustado ao que foi feito; continuação de "pode seguir")
- **Objetivo:** critério 11 da rubrica: system prompts documentados com racional e um ciclo real de refinamento com problema, alteração e resultado medido.
- **Prompt:**
  > Com os problemas reais registrados na rodada v1 (confiança saturada em 1.0 inclusive para indefinido, passos da ação colados sem separador, equipe "Recursos Humanos" inventada no exemplo 07), escreva o prompt v2 em src/triagem/prompts.py atacando cada um: confiança calibrada por faixas com ≤ 0.5 obrigatório para indefinido, ambiente delegado às regras da aplicação, passos um por linha e proibição explícita de nomear equipe ou departamento fora do contexto. Preserve as âncoras que os testes de segurança verificam. Rode os mesmos 7 exemplos com o Gemini, salve em docs/evidencias/execucoes/prompt-v2/ com README, e escreva docs/refinamento-prompt.md com o diff real do git, tabelas antes/depois (confiança, formato, equipe) geradas a partir das saídas salvas e o que não mudou como controle. Gere docs/instrucoes-agente.md a partir das constantes do código, com racional por regra e ligação com os requisitos do PRD, e crie um teste que falhe se a documentação divergir do código. Confirme ou recalibre LIMIAR_CONFIANCA com os valores do v2.
- **Resultado:** prompts v2 (4 mudanças, diff de 22 linhas). Nos 7 exemplos: confiança 1.0 → 0.95 nos técnicos e 1.0 → 0.5 no fora do domínio (`confianca_baixa` passa a disparar); nenhuma ação com passos colados; 07 sem equipe inventada; rotas e prioridades inalteradas. Os 7 cenários batem com `docs/cenarios.md` pela primeira vez com o modelo real. `LIMIAR_CONFIANCA` confirmado em 0.6. `docs/instrucoes-agente.md` gerado do código e protegido por `tests/test_prompts_docs.py` (370 testes). PRD 1.6.

### P-039 · Evidências consolidadas em docs/evidencias com índice e rastreabilidade (issue #19)
- **Origem:** usuário ("pode seguir", após o resumo do estado do projeto com as 4 issues restantes) + prompt sugerido na issue, ajustado ao que já existia
- **Objetivo:** critérios 6, 7, 8, 9, 12 e 13 da rubrica: reunir as provas de funcionamento num só lugar, explicar o que cada arquivo prova e garantir que nada contém segredo ou dado pessoal.
- **Prompt:**
  > As execuções reais dos 7 exemplos já estão em docs/evidencias/execucoes/prompt-v1/ e prompt-v2/ (issues #11, #16 e #17), então não regenere nada. Complete a issue #19: gere docs/evidencias/testes.txt com a saída integral de `uv run pytest -v` (incluindo os testes live com o Gemini), substituindo o caminho local da máquina; registre a última execução verde do CI na main sobre o código final a partir da API do Actions (jobs, etapas, contagem de testes por versão do Python e histórico de todas as execuções, com as falhas explicadas); escreva docs/evidencias/README.md como índice explicando cada arquivo, como ler um par .json/.jsonl, o que cada evento de log prova e um mapa de cada um dos 13 critérios da rubrica para o arquivo que o comprova; em docs/extensoes.md troque a frase de que a evidência real do exemplo 06 está pendente pelos links e pelo que os logs mostram, e acrescente os registros do CI e o testes.txt à E1; atualize o PRD (RF-52 e seções 9, 13 e 15) para o layout real, pares por cenário e por versão de prompt em vez de logs/ e saidas/. Antes de fechar, varra a pasta procurando chaves, e-mails, CPFs e caminhos pessoais e registre o resultado no índice.
- **Resultado:** `docs/evidencias/testes.txt` (372 testes verdes, 2 deles `live` com o Gemini real); `docs/evidencias/ci/execucao-main-2026-09-16.md` (execução #36, 370 testes em Python 3.10 e 3.12; 34 das 36 execuções verdes, as 2 falhas em PR e corrigidas antes do merge); `docs/evidencias/README.md` com mapa da pasta, leitura dos logs e rastreabilidade dos 13 critérios; `docs/extensoes.md` sem pendência na E2; PRD 1.7. Varredura sem chave, CPF ou caminho pessoal; os únicos e-mails são os fictícios `@empresa.exemplo` do catálogo, nas saídas. Nenhum arquivo de execução foi regenerado ou editado.

### P-040 · README completo com os 11 itens da seção 16 do PRD (issue #18)
- **Origem:** continuação de "pode seguir" (ordem proposta #19 → #18 → #20 → #21) + prompt sugerido na issue, ajustado ao que já existia nos docs
- **Objetivo:** critério 3 da rubrica: guia único para compreender, configurar, executar, testar e avaliar a solução; uma pessoa sem contexto executa o projeto em clone limpo só seguindo o README.
- **Prompt:**
  > Escreva o README.md com os 11 itens da seção 16 do docs/PRD.md, nesta ordem: objetivo da triagem e o que a saída estruturada contém; arquitetura com o diagrama Mermaid gerado por `uv run triagem grafo` (verbatim, sem redesenhar) e a relação com o fluxo da especificação; state, tabela dos 7 nodes com tipo, função e falha tratada, as três decisões condicionais, as regras determinísticas e a condição de parada com os testes que a provam; tool com contrato, quando é chamada, fonte de dados e tabela de falhas; estratégia de contexto (state + base + BM25 + uso na resposta) e por que não embeddings; instalação, tabela das variáveis do .env.example, comandos copiáveis de execução e de testes com códigos de saída, tabela de testes por arquivo com as contagens de docs/evidencias/testes.txt; cenários com as saídas reais salvas em docs/evidencias/execucoes/prompt-v2/ (01 em JSON, 02 em --formato texto gerado a partir do JSON salvo pela função da CLI, 04 e 05 resumidos), sem chamar o modelo de novo; observabilidade com tabela de eventos e trecho real do log do 02; links para qa-com-ia, refinamento-prompt, instrucoes-agente e prompts com o que cada um contém; extensões E1 e E2 com evidências e badge; limitações conhecidas consolidadas dos docs (detector lexical, termos por substring, confiança autoavaliada, não determinismo, BM25, escopo single-shot, provedor único, PII parcial); placeholder do vídeo para a issue #20; estrutura do repositório real. Depois valide em clone limpo: uv sync, pytest, ruff, triagem grafo, cenário 04 sem .env, chave ausente com código 2 e um cenário real com o Gemini.
- **Resultado:** README de 13 seções com sumário; diagrama e saídas copiados das evidências (nenhuma nova chamada ao modelo para redigir); tabela de 372 testes por arquivo; 8 limitações. Validação em clone limpo registrada no comentário da issue #18.

## Fase 6 — Vídeo e entrega (iniciada em 16/09/2026)

### P-041 · Roteiro do vídeo com comandos exatos e mapeamento para o item 5.4 (issue #20, parte do assistente)
- **Origem:** continuação de "pode seguir" + prompt sugerido na issue #20, restrito ao que o assistente pode fazer (gravação, publicação e link são do aluno)
- **Objetivo:** critério 1 da rubrica: garantir que o vídeo de até 10 minutos mostre os 8 itens do item 5.4 da especificação, com comandos que já foram executados e saídas conhecidas.
- **Prompt:**
  > Extraia do docx da especificação os 8 itens do item 5.4 e, com base na seção 17 do PRD, escreva docs/roteiro-video.md com meta de 9 minutos: uma tabela mapeando cada item do 5.4 ao bloco em que aparece e ao que apontar na tela; preparação antes de gravar (LOG_FORMATO=texto, aquecer os comandos, nunca mostrar o .env, abas do navegador); blocos com tempo acumulado, o comando exato, o que apontar no log e na saída (usando os nomes reais dos eventos e os valores das execuções salvas) e o que falar; e o checklist de depois de gravar (duração, não listado, janela anônima, link no README por PR que fecha a #20, link para o AVA). Aponte a seção 12 do README para o roteiro sem tirar o placeholder do link. Abra o PR sem fechar a issue e registre na issue o que fica para o aluno.
- **Resultado:** `docs/roteiro-video.md` com 8 blocos (A a H, 0:00 a 9:00), 6 comandos, todos já executados nas evidências; README §12 e estrutura atualizados. Pendente para o aluno: ensaio, gravação, publicação e link.

### P-042 · Revisão final em clone limpo, varredura do histórico e checklist da especificação (issue #21, parte do assistente)
- **Origem:** continuação de "pode seguir" + prompt sugerido na issue #21, restrito ao que não depende do aluno (acesso do professor, vídeo e submissão no AVA ficam com ele)
- **Objetivo:** critérios 1, 2 e 3 da rubrica: garantir que a `main` final está íntegra, reproduzível e sem segredos, e deixar explícito o que ainda falta para submeter.
- **Prompt:**
  > Faça a revisão final do repositório: clone limpo da main em pasta vazia sem .env, uv sync, pytest, ruff, triagem grafo, cenário 04 sem chave, chave ausente com código 2 e os 7 exemplos com o Gemini real, comparando com docs/cenarios.md. Varra todo o histórico git (todas as branches) por arquivos .env, chaves e tokens, e confira visibilidade e colaboradores do repositório. Extraia do docx a seção 7 (checklist final) e escreva docs/checklist-entrega.md percorrendo cada item com status (atendido, pendente do aluno, atenção) e a evidência que o comprova, mais a lista do que falta antes da submissão no AVA. Abra o PR sem fechar a issue e registre na issue o que fica para o aluno.
- **Resultado:** `docs/checklist-entrega.md`: 28 itens da especificação, 24 atendidos e verificados, 3 pendentes do aluno (vídeo e submissão) e 1 em atenção (repositório **privado** com um só colaborador, o professor ainda não tem acesso). Clone limpo verde; 7 exemplos batendo com `cenarios.md`; histórico sem `.env` nem chave real (só a chave fictícia de `tests/test_seguranca.py`).

### P-043 · PRD 1.8: seção 13 alinhada ao repositório, RF-71/72 e Q2/Q3 fechados (issue #21, higiene)
- **Origem:** usuário ("tem alguma pendencia de implementação?" → "pode fazer"), após a verificação de que não havia pendência de código
- **Objetivo:** deixar o PRD coerente com o repositório final para a avaliação: nenhum arquivo citado que não exista, nenhum requisito entregue sem anotação, nenhuma questão aberta que já foi resolvida.
- **Prompt:**
